...
from .audio import AudioManager, AudioAnalyzer
from .analysis import compute_background_brightness, compute_brightness_stats
from .utils import (
    FrameCache,
    FRAME_CACHE_SIZE,
...
<b>Analysis Range:</b> {self.start_frame + 1}-{(self.end_frame or 0) + 1}
    ...
    def _update_current_brightness_display(self):
        ...
        if self.background_roi_idx is not None:
            background_brightness = compute_background_brightness(self.frame, self.rects, self.background_roi_idx)
        ...
        for idx, (pt1, pt2) in enumerate(self.rects):
            ...
            if x2 > x1 and y2 > y1: # Check for valid ROI area
                ...
                brightness_stats = compute_brightness_stats(roi, background_brightness)
        ...
        if roi_data:
            ...
            if self.background_roi_idx is not None and background_brightness is not None:
                ...
                if bg_x2 > bg_x1 and bg_y2 > bg_y1:
                    ...
                    _, _, _, _, bg_b_mean, _, _, _ = compute_brightness_stats(bg_roi)
    ...
    def _calculate_background_threshold(self) -> Optional[float]:
        ...
        if self.background_roi_idx is not None and self.frame is not None:
            ...
            if 0 <= self.background_roi_idx < len(self.rects):
                ...
                if x2 > x1 and y2 > y1:
                    roi = self.frame[y1:y2, x1:x2]
                    l_raw_mean, _, _, _, _, _, _, _ = compute_brightness_stats(roi)
                    return l_raw_mean
        ...
        try:
            ...
            for f_idx in range(start, end + 1):
                ...
                for data_idx, roi_idx in enumerate(non_background_rois):
                    ...
                    if x2 > x1 and y2 > y1:
                        roi = frame[y1:y2, x1:x2]
                        l_raw_mean, l_raw_median, l_bg_sub_mean, l_bg_sub_median, b_raw_mean, b_raw_median, b_bg_sub_mean, b_bg_sub_median = compute_brightness_stats(roi, background_brightness)
