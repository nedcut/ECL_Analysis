"""Audio analysis for detecting beeps and completion signals in video."""
...
class AudioAnalyzer:
    ...
    def detect_beeps(
        ...
        try:
            ...
            logging.info(
                f"Targeting {target_frequency}Hz ±{frequency_tolerance}Hz "
                f"({min_frequency:.1f}-{max_frequency:.1f}Hz)"
            )
            ...
            times = librosa.frames_to_time(
                np.arange(len(energy_per_frame)),
                sr=sample_rate,
    ...
    def find_completion_beeps(
        ...
        try:
            ...
            if expected_run_duration > 0.0:
                ...
                if filtered_results:
                    logging.info(
                        f"Filtered {len(results)} beeps to {len(filtered_results)} "
                        f"based on run duration"
                    )
