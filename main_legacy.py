...
DEFAULT_BRIGHTNESS_NOISE_FLOOR = 10.0    # ← NEW
...
class VideoAnalyzer(QtWidgets.QMainWindow):  # Changed to QMainWindow for better menu support
    ...
    def _init_vars(self):
        ...
        self.brightness_noise_floor = DEFAULT_BRIGHTNESS_NOISE_FLOOR  # ← NEW
...
<b>Ctrl+Q:</b> Exit application<br>
    ...
    def _create_widgets(self):
        ...
        # Brightness noise floor controls (NEW)
        noise_layout = QtWidgets.QHBoxLayout()
        noise_layout.addWidget(QtWidgets.QLabel("Noise Floor L*:", self))
        self.noise_floor_spin = QtWidgets.QDoubleSpinBox(self)
        self.noise_floor_spin.setDecimals(1)
        self.noise_floor_spin.setRange(0.0, 50.0)
        self.noise_floor_spin.setSingleStep(0.5)
        self.noise_floor_spin.setValue(self.brightness_noise_floor)
        noise_layout.addWidget(self.noise_floor_spin)
        th_layout.addLayout(noise_layout)
    ...
    def _connect_signals(self):
        ...
        self.noise_floor_spin.valueChanged.connect(self._on_noise_floor_changed)  # NEW
...
<b>Analysis Range:</b> {self.start_frame + 1}-{(self.end_frame or 0) + 1}
    ...
    def _on_noise_floor_changed(self, value: float):
        """Handle changes to the noise floor spinbox."""
        self.brightness_noise_floor = value
...
Frames Analyzed: {len(frames)}"""
    ...
    def _compute_brightness_stats(self, roi_bgr: np.ndarray) -> Tuple[float, float]:
        ...
        try:
            ...
            # Keep only pixels with brightness > noise floor
            mask = l_star > self.brightness_noise_floor
            if not np.any(mask):
                return 0.0, 0.0
